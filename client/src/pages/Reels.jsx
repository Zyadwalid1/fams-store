...
const Reels = () => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/products/${currentReel.product._id}?populate=type,subtype`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/reels/like/${currentReel._id}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
