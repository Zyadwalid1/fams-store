...
export const AuthProvider = ({ children }) => {
  ...
  const getLoginHistory = async (page = 1, limit = 10) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/users/login-history?page=${page}&limit=${limit}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`
        }
      });
  ...
  const getAddresses = async () => {
    try {
      ...
      console.log('Fetching addresses from: http://localhost:5000/api/users/addresses');
  ...
  const addAddress = async (addressData) => {
    try {
      ...
      console.log('Adding address at: http://localhost:5000/api/users/addresses');
  ...
  const updateAddress = async (addressId, addressData) => {
    try {
      ...
      console.log(`Updating address at: http://localhost:5000/api/users/addresses/${addressId}`);
  ...
  const deleteAddress = async (addressId) => {
    try {
      ...
      console.log(`Deleting address at: http://localhost:5000/api/users/addresses/${addressId}`);
