...
const ReelsTab = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/products?populate=type,subtype', {
        headers: {
  ...
  const fetchReels = async (page = 1) => {
    try {
      setIsLoading(true);
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`http://localhost:5000/api/reels?page=${page}&limit=10`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleFileUpload = async (file, type) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/reels/upload/${type}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = selectedReel 
        ? `http://localhost:5000/api/reels/${selectedReel._id}`
  ...
  const handleDelete = async (reelId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`http://localhost:5000/api/reels/${reelId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
